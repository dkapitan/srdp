...
A project maps onto a Dagster code location (reusing the existing `projects/` directory). The project's IO manager resource is configured with that project's catalog, so the catalog is supplied by the code location, not encoded in every asset key.
...
- Good, because projects reuse Dagster code locations and the existing `projects/` layout, so the catalog dimension stays out of asset keys.
