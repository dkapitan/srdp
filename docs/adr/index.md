...
| [0008 — Identity propagation, capability tokens, and data contracts](./0008-identity-propagation-and-data-contracts.md) | Proposed | Validated JWTs at every hop, PEP-issued short-lived capability tokens, data contracts as the fine-grained scope artifact, constrained evaluator, OData consumer endpoint |
| [0009 — Data-plane access: credential materialization](./0009-data-plane-credential-materialization.md) | Proposed | Capability token → scoped engine/storage/compute credentials; credentials subordinate to the token, fine-grained scope mediated-only, compute as a distinct capability |
